...
export function SiteHeader({
  ...
  return (
    <header className="bg-card border-b relative overflow-hidden">
      <div 
        className="absolute inset-0 bg-no-repeat bg-cover bg-center pointer-events-none select-none mix-blend-multiply dark:mix-blend-screen opacity-[0.05] dark:opacity-[0.02]"
        style={{ backgroundImage: "url('/28.jpg')" }}
      />
...
function KpiTile({
  ...
  return (
    <Card
      className="px-3 py-2 shadow-none border-border/60 transition-colors hover:border-primary/40"
      title={tip}
    >
      <div className="text-[10px] uppercase tracking-wider text-muted-foreground">
        {label}
      </div>
      <div className={`text-lg font-semibold num ${tone}`}>{children}</div>
    </Card>
  );
}
  ...
  return (
    <div className="bg-amber-100/80 border-t border-amber-200 dark:bg-amber-950/30 dark:border-amber-900">
      <div className="mx-auto max-w-7xl px-6 py-2 flex items-center justify-between text-xs text-amber-950 dark:text-amber-200 font-medium">
        ...
        <div className="hidden md:block text-amber-800 dark:text-amber-400 font-medium">
          Constraint set by operations leadership.
        </div>
      </div>
    </div>
  );
